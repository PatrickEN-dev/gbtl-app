...
## Spec source
Read `sdd/specs/data.md` — contains `## HOOKS` with exact implementations for useCart, useAuth, useBottomSheet and useProducts/useProduct patterns.
Read `sdd/specs/design.md` — contains `## DESIGN PATTERNS` for custom hook encapsulation pattern.
Also read wave 1 outputs: `src/types/index.ts`, `src/services/products.ts`, `src/services/auth.ts`, `src/store/*.ts`
